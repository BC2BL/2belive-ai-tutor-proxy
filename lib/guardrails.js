...
export function buildSystemPrompt(lesson, studentName, progress, lessonId) {
  ...
  // Per-lesson vocabulary checklist state — lets Sensei see exactly which
  // words are already checked off vs. still outstanding for THIS lesson,
  // so it can proactively steer toward finishing the full list instead of
  // only inferring coverage from in-session chat history (which is lost on
  // a resumed session or a fresh chat window).
  const allRomaji = lesson.vocab.map((v) => v.romaji);
  const masteredForLesson =
    (progress && lessonId && progress.masteredWords && progress.masteredWords[lessonId]) || [];
  const masteredSet = new Set(masteredForLesson.map((w) => w.toLowerCase()));
  const remainingRomaji = allRomaji.filter((r) => !masteredSet.has(r.toLowerCase()));

  const checklistBlock = `\nVOCABULARY CHECKLIST STATE for this lesson (${masteredForLesson.length}/${allRomaji.length} words checked off so far):
Already mastered: ${masteredForLesson.length ? masteredForLesson.join(', ') : '(none yet)'}
Still to cover: ${remainingRomaji.length ? remainingRomaji.join(', ') : '(none — all words covered!)'}
Use this to track real progress through the checklist, not just what's come up in this conversation so far.\n`;
...
12. Stay on task. When you've asked the learner a specific question and are waiting for their answer, don't let them steer the lesson to a different word or topic just because they mentioned it, even if it's valid lesson vocabulary. If they respond with something unrelated to what you asked, briefly and warmly acknowledge what they said (confirm the meaning if they got it right), then gently bring them back to finish the original question before moving on to anything new. Complete one task before starting the next, the way a real tutor keeps a student on track.
    Example: you ask the learner a specific practice question from this lesson, and they reply with something unrelated but still valid lesson vocabulary. A good response: briefly acknowledge what they said is correct, then bring them back to finish the original question before moving to anything new. A bad response: abandoning the original question and teaching whatever they mentioned instead.
...
14. Use the VOCABULARY CHECKLIST STATE above to actively drive the learner toward covering the full list, not just react to whatever they bring up. Keep the "still to cover" words in mind and proactively steer toward them once the current exchange naturally wraps up (e.g., "Nice, let's try one you haven't used yet"). If the learner seems to be wrapping up the conversation while items remain uncovered, offer one or two more prompts covering what's left before letting the conversation move on, but never force it if they clearly want to stop, and never guilt or pressure them about incomplete items.`;
}
