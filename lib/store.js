...
export const store = {
  ...
  async getHistory(studentId, lessonId) {
    const raw = await kvGet(historyKey(studentId, lessonId));
    if (raw === null) return [];
    try {
      const parsed = JSON.parse(raw);
  ...
  async appendHistory(studentId, lessonId, userMessage, assistantReply) {
    const history = await this.getHistory(studentId, lessonId);
